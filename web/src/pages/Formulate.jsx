...
export default function Formulate() {
  ...
  function downloadMarkdown() {
    ...
    const lines = [
      `# IntelliForm Formulation Proposal`,
      `**Date:** ${now}  **Vertical:** ${vertical}  **Batch size:** ${batchSize} kg`,
      '',
      `## Brief`,
      inputText || '—',
      '',
      ...
      `| Metric | Value |`,
      `|--------|-------|`,
      `| Cost per kg | ${m.cost_per_kg != null ? `$${Number(m.cost_per_kg).toFixed(2)}` : '—'} |`,
      `| Bio-based share | ${m.bio_pct != null ? `${Number(m.bio_pct).toFixed(1)}%` : '—'} |`,
      `| Performance score | ${m.perf_score != null ? Number(m.perf_score).toFixed(1) : '—'} |`,
      `| Eco grade | ${(typeof eco === 'object' ? eco.grade : null) || '—'} |`,
      `| Eco score | ${(typeof eco === 'object' ? eco.eco_score : null) ?? '—'} |`,
      '',
      `## Blend Composition`,
      `| Ingredient | Weight % | CAS | MW (g/mol) |`,
      `|------------|----------|-----|------------|`,
      ...blendEntries.map(([name, pct]) => {
        const pc = pubchem[name] || {}
        return `| ${name} | ${pct} | ${pc.cas || ''} | ${pc.molecular_weight || ''} |`
      }),
      '',
    ]
    if (carbon.summary) {
      lines.push(`## Carbon & Circularity`)
      lines.push(carbon.summary)
      lines.push('')
      lines.push(`| Metric | Value |`)
      lines.push(`|--------|-------|`)
      if (carbon.co2_displaced_kg != null) lines.push(`| CO2 displaced | ${carbon.co2_displaced_kg} kg/batch |`)
      if (carbon.green_co2_per_kg != null) lines.push(`| Green CO2e | ${carbon.green_co2_per_kg} kg/kg |`)
      if (carbon.baseline_co2_per_kg != null) lines.push(`| Petro baseline CO2e | ${carbon.baseline_co2_per_kg} kg/kg |`)
      if (carbon.circular_grade) lines.push(`| Circular grade | ${carbon.circular_grade} |`)
      lines.push('')
    }
    const reg = result.regulatory
    if (reg?.summary) {
      lines.push(`## Regulatory Summary`)
      lines.push(reg.summary)
      lines.push('')
    }
    lines.push(`---`)
    lines.push(`*Generated by IntelliForm — ChemeNova LLC*`)
    const blob = new Blob([lines.join('\n')], { type: 'text/markdown' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a'); a.href = url; a.download = 'intelliform_proposal.md'; a.click()
